"""
Word (.docx) bazani o'qish.
Har savol uchun IKKI ko'rinish saqlanadi:
  • MATN  -> Telegram interaktiv rejimi uchun (formulalar Unicode'ga aylanadi)
  • XML   -> Word chiqishi uchun (formulalar/asl bezak AYNAN ko'chiriladi, o'zgartirilmaydi)
Format: savol oldida #N. , to'g'ri javob oldida +
"""
import re
import io
import copy
...
def _split_options(stream):
    """content node oqimidan variantlarni ajratadi -> [{correct, nodes:[lxml]}]."""
    options = []
    cur = None
    pending_plus = False
    for node in stream:
